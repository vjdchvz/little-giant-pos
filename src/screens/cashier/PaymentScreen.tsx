...
// Little Giant POS — Payment Screen
...
export default function PaymentScreen() {
  ...
  const handleCharge = async () => {
    try {
      ...
      // Dev fallback — simulate successful order
      const mockOrder = {
        id: Date.now(),
      ...
      addOrder(mockOrder);
      clearCart();
      navigation.replace('OrderSuccess', { order: mockOrder });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          style={[styles.chargeBtn, loading && styles.chargeBtnDisabled]}
        ...
        >
          {loading
            ...
            : <>
                ...
                <Text style={styles.chargeBtnText}>Charge ₱{total.toFixed(2)} via {payMethod.toUpperCase()}</Text>
              </>
          }
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  sectionLabel:       { fontSize: Typography.xs, fontWeight: Typography.medium, color: Colors.textMuted, letterSpacing:0.5 },
